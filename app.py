...
def dashboard():
    ...
    total_bots = len(bots)
    running_bots = len([b for b in bots if b.bot_status == 'running']) if bots else 0
...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        ...
        log_activity(user.id, 'upload', f'Uploaded {original_filename} (Platform: {platform})')
...
@app.route('/extracted_files/<int:file_id>')
@login_required
def view_extracted_files(file_id):
    ...
    return render_template('extracted_files.html', file=file_record, files=extracted_files)
...
def start_bot(bot_id):
    ...
    try:
        ...
        log_activity(user.id, 'start_bot', f'Started bot: {bot.bot_name}')
...
def stop_bot(bot_id):
    ...
    try:
        ...
        log_activity(user.id, 'stop_bot', f'Stopped bot: {bot.bot_name}')
...
def deploy_bot():
    ...
    log_activity(user.id, 'deploy_bot', f'Deployed bot: {bot_name} (Platform: {platform})')
...
def delete_bot(bot_id):
    ...
    log_activity(user.id, 'delete_bot', f'Deleted bot: {bot.bot_name}')
...
def get_processes():
    ...
    with process_lock:
        for bot_id, info in running_processes.items():
            ...
            if bot and bot.user_id == user.id:
                try:
...
def get_bot_templates():
    ...
    templates = [
        {'id': 1, 'name': 'Python Discord Bot', 'type': 'python', 'description': 'Complete Discord bot with moderation features', 'icon': '🐍', 'version': '3.11', 'template_code': 'import discord\n\nclient = discord.Client()\n\n@client.event\nasync def on_ready():\n    print("Bot is ready!")'},
        {'id': 2, 'name': 'Node.js Web Server', 'type': 'nodejs', 'description': 'Express.js web server with REST API', 'icon': '💚', 'version': '20.x', 'template_code': 'const express = require("express");\nconst app = express();\n\napp.get("/", (req, res) => {\n    res.send("Hello World!");\n});\n\napp.listen(3000);'}
    ]
...
def api_deploy_bot():
    ...
    # Create a simple bot record for template deployment
    new_bot = UserBot(
        bot_name=bot_name,
        bot_type='python',
        bot_status='stopped',
        ...
        user_id=user.id,
        file_id=None,
        config=json.dumps({'template_id': template_id, 'deployed_at': datetime.utcnow().isoformat()})
    )
    ...
    log_activity(user.id, 'deploy_bot', f'Deployed bot from template: {bot_name}')
...
def public_stats():
    ...
    return jsonify({'totalUsers': total_users, 'totalBots': total_bots})
